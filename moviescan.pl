...
my $base_output_dir = "/var/lib/mythtv/videos/movies";
	...
	die "Can't find $base_output_dir" unless -d $base_output_dir;
...
for my $top_dir ( @ARGV ) {
	...
	while( defined( my $movie_dir = readdir $dh ) ) {
		...
		my $physical_path = "$top_dir/$movie_dir/$movie_file";
		my $movie_name = $movie_dir; 

		#my $physical_path = "movies/$movie_dir/$movie_file"; #TODO Fix the hardcoding of movies/
		#TODO Add option to skip this check
		if( movie_in_db( "movies/all/$movie_name" ) ) { 
		...
		mk_link( $physical_path, $movie_name, 'all' ) or next;

		$movie_data->{filename} = "movies/all/$movie_name";
		insert_movie( $movie_data );

		for( @{ $info->{genres} } ) {
			mkdir "$base_output_dir/genre";
			mk_link( $physical_path, $movie_name, "genre/$_->{name}" );

			$movie_data->{filename} = "movies/genre/$_->{name}/$movie_name";
			insert_movie($movie_data);
		}
